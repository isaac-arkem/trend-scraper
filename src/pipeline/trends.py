...
def _save_clip(clip: dict, sound_id: Optional[str]) -> Optional[dict]:
    ...
    # Derived niche, kept separate from topic on purpose: topic means "came from
    # a curated reference account" and Reference Profiles filters on it, so a
    # guess written there would be indistinguishable from a human's choice.
    if clip.get("niche"):
        row["niche"] = clip["niche"]
