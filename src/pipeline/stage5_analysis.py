...
CAP_PER_CREATOR = 20
# If the first few images show nobody, this creator is not a casting candidate
# and the remaining images will not change that. Without this, a creator whose
# feed is products, text posts or scenery burned the full 20-image cap before
# stopping — roughly $0.07 wasted per dead creator, and dead creators are common
# in hashtag discovery.
ABANDON_AFTER = 3        # images checked before giving up on a creator
...
def analyze_creator(creator_id: str, cap: int = CAP_PER_CREATOR,
    ...
    for asset in pending:
        if good >= good_reads_to_stop or analyzed >= cap:
            ...
            continue
        # give up early: nothing usable in the first ABANDON_AFTER images
        if good == 0 and checked_here >= ABANDON_AFTER:
            leftover.append(asset["id"])
            continue
