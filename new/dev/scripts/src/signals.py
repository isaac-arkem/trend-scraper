...
# Hashtags that ride on almost every clip and so rank at the top of every
# country's board while saying nothing. Filtering these at SEED time was not
# enough: the board ranks tags found ON the harvested clips, not the tags we
# searched for, so #fyp and #dance came straight back in through the captions.
BOARD_STOP_TAGS = {
    "fyp", "fypシ", "fypage", "foryou", "foryoupage", "foryourpage", "fy",
    "viral", "viralvideo", "viraltiktok", "trending", "trend", "trendingnow",
    "tiktok", "tiktokviral", "explore", "explorepage", "xyzbca", "parati",
    "capcut", "capcutpioneer", "duet", "greenscreen", "edit",
    "dance", "dancetok", "dancechallenge", "dancetrend", "dancetrends",
    "tiktokdance", "dancechallenge2026", "newdancechallenge", "trendingdance",
    "brazildance", "dance2026", "dancer", "dancing", "dancevideo", "dancemoves",
    "braziliandance", "blowthisup", "tryit", "reels", "reelsdance", "reel",
}
...
def rank_hashtags(clips, baselines, days=None, limit=50, min_bucket=None, breakout_at=None):
    return _bucket_rows(
        clips, baselines,
        key_fn=lambda c: [f"#{h.lower()}" for h in (c.get("hashtags") or [])
                          if h and h.lower().lstrip("#") not in BOARD_STOP_TAGS],
